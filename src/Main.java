    ...
    public static void main(String[] args) {
        for (int i = 0; i < AMOUNT_OF_THREADS; i++) {
            new Thread(() -> {
                ...
                int freguency = (int) route.chars().filter(ch -> ch == 'R').count();

                synchronized (sizeToFreq) {
                    if (sizeToFreq.containsKey(freguency)) {
                        sizeToFreq.put(freguency, sizeToFreq.get(freguency) + 1);

                    } else {
                        sizeToFreq.put(freguency, 1);
                    }
                }
            }).start();
        }
